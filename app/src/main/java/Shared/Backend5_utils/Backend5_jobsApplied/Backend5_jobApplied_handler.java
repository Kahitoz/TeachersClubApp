...
public class Backend5_jobApplied_handler {
    ...
    public Backend5_jobApplied_handler(Context context, FirebaseAuth auth, FirebaseFirestore database, RecyclerView b5_recyclerView, ProgressBar b5_progress_bar) {
                ...
                .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            List<Backend5_jobApplied_GetterSetter> documentList = new ArrayList<>();

                            for (DocumentSnapshot document : task.getResult()) {
                                Backend5_jobApplied_GetterSetter backend5Document = document.toObject(Backend5_jobApplied_GetterSetter.class);
                                documentList.add(backend5Document);
                            }

                            Backend5_jobApplied_Adapter adapter = new Backend5_jobApplied_Adapter(documentList, context);
